...
function Index() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      const response = await axios.get('https://onlineshop-backend-bspd.onrender.com/products');
        ...
        <span className="badge bg-danger mb-3 bg-4">Discount</span>
          <Slider ref={sliderRef} {...settings}>
            {discountProducts.map((product) => (
              <div key={product._id}>
                <img src={`https://onlineshop-backend-bspd.onrender.com/uploads/${product.image}`} alt={product.name} className='slider-img'/>
                <h5>{product.name}</h5>
                <p>${product.price}</p>
              </div>
            ))}
          ...
          {products.map((product) => (
            <div key={product._id} className="col-md-4 mb-4">
              <div className="card product-box">
                <img src={`https://onlineshop-backend-bspd.onrender.com/uploads/${product.image}`} alt={product.name} className="card-img-top" />
